...
export const fetchUsers = createAsyncThunk<any[]>('user/fetchUsers', async () => {
   return await userService.getAll();
});

// 🔹 eingeloggten User aktualisieren (/users/me)
export const updateUser = createAsyncThunk<any, Partial<User>, { state: RootState }>(
   'user/update',
...
export const updateUserById = createAsyncThunk<any, { id: number; changes: any }, { state: RootState }>(
   'user/updateById',
...
export const deleteUser = createAsyncThunk<number, number | undefined>('user/deleteUser', async (id) => {
   if (id != null) {
   ...
   extraReducers: (builder) => {
      builder
         ...
         .addCase(fetchUsers.fulfilled, (state, action: PayloadAction<any[]>) => {
            ...
            console.log('[userSlice] fetchUsers.fulfilled – raw payload:', action.payload);

            state.users = action.payload.map(mapApiUserToUser);

            console.log('[userSlice] fetchUsers.fulfilled – normalized users:', state.users);
         })
