...
export const signup = catchAsync(async (req: Request, res: Response) => {
    ...
    if (requiredErrors.length) {
        return res.status(400).json({ error: `Folgende Pflichtfelder fehlen oder sind leer: ${requiredErrors.join(', ')}` });
    }
    ...
    return res.status(201).json({ message: 'Signup erfolgreich. Bitte prüfe deine E-Mail zur Verifikation.' });
});
...
function extractRefreshToken(req: Request): string | undefined {
    ...
    const c = req.cookies?.refreshToken;
    if (c && c.length > 0) return c;

    // 2) Fallback: Rohheader parsen und den letzten nicht-leeren nehmen
    const raw = req.headers?.cookie;
    if (!raw) return undefined;
    const parts = raw.split(';').map(s => s.trim()).filter(s => s.startsWith('refreshToken='));
...
function clearAuthCookiesEverywhere(res: Response, req: Request) {
    const base = { sameSite: 'lax' as const, secure: req.protocol === 'https' };
...
export const refresh = catchAsync(async (req: Request, res: Response) => {
    ...
    res.cookie('refreshToken', newRefreshToken, {
        httpOnly: false,       // clientseitig gesetzt => hier false
        sameSite: 'lax',
...
export const verifyEmail = catchAsync(async (req: Request, res: Response) => {
    ...
    return res.status(200).json({ message: 'E-Mail erfolgreich bestätigt. Du kannst dich jetzt einloggen.' });
});

/** POST /api/auth/resend */
export const resendVerification = catchAsync(async (req: Request, res: Response) => {
    const { userId, email, firstName } = req.body;
    if (!userId || !email) return res.status(400).json({ error: 'userId und email erforderlich' });

    await authService.resendVerification(Number(userId), String(email), String(firstName));
    res.status(200).json({ message: 'Verifikations-E-Mail wurde erneut gesendet (falls zulässig).' });
});
